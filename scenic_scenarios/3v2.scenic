...
# behavior selector
behavior random_behavior(ball, behavior_p):
    if behavior_p <= 0.25:
        do goToMidPoint()
    elif behavior_p <= 0.5:
        do helpGK()
    elif behavior_p <= 0.75:
        do FollowObj(ball)
    else:
        do FollowPersonWithBall(ball)

# Right
o0 = RightGK at 98 @ 0
o1 = RightCB at (p1 offset along -1*Range(60,120) deg by 0 @ Range(10, 30)) , with behavior random_behavior(ball, Range(0,1))
